...
public class BoxVolume {
    ...
    public BoxVolume(float halfWidth, float halfHeight, float halfDepth) {
    ...
    public BoxVolume setHalfExtents(float halfWidth, float halfHeight, float halfDepth) {
    ...
    public boolean intersects(BoxVolume other, Transform transform, Transform otherTransform) {
        if (other == null) throw new IllegalArgumentException("Other volume cannot be null");
        if (transform == null) throw new IllegalArgumentException("Transform cannot be null");
        if (otherTransform == null) throw new IllegalArgumentException("Other transform cannot be null");

        Vector3f center = transform.getWorldPosition();
        Vector3f otherCenter = otherTransform.getWorldPosition();
        Vector3f otherHalfExtents = other.getHalfExtents();

        return Math.abs(center.x - otherCenter.x) <= halfExtents.x + otherHalfExtents.x
            && Math.abs(center.y - otherCenter.y) <= halfExtents.y + otherHalfExtents.y
            && Math.abs(center.z - otherCenter.z) <= halfExtents.z + otherHalfExtents.z;
    }
}
